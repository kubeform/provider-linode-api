...
gen-apis:
	...
	@provider-$(PROVIDER)-gen --apis-path=$$(pwd)
...
label-crds: $(BUILD_DIRS)
	@for f in crds/*.yaml; do \
		...
		mv bin/crd.yaml $$f; \
		kubectl label --overwrite -f $$f --local=true -o yaml app.kubernetes.io/name=$(PROVIDER).kubeform.com > bin/crd.yaml; \
		mv bin/crd.yaml $$f; \
	done
